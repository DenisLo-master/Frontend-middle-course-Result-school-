...
type NormalizedPost = Record<string, Post>

interface NormalizeDataPost {
  byId: NormalizedPost
  allIds: string[]
}
...
export const normalizeData = (unnormalizedData: Post[]): NormalizeDataPost => {
  const byId: NormalizedPost = {}
  const allIds: string[] = []
  ...
  const normalizedPosts = { byId, allIds }
  return normalizedPosts as NormalizeDataPost
}
